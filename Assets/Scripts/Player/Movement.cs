...
public void HandleAction(Player player)
    ...
    private void ExtinguishFireAt(Vector3 firePos, string action)
    {
        ...
        GameObject[] collidedGameObjects = Physics.OverlapSphere(firePos, detectionRadius)
        .Except(new[]
        ...
        foreach (GameObject collision in collidedGameObjects)
        {
            ...
            {
                Debug.Log($"Ejecutando {action}");
                if (action == "removeFire")
                {
                    Destroy(collision);
                    return;
                }
                ...
                {
                    Destroy(collision);
                    Instantiate(smokePrefab, firePos, Quaternion.identity);
                    return;
                }
